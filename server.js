...
    <head>
        <meta charset="UTF-8">
        <!-- هذا السطر هو الذي يحل مشكلة شكل الموقع في الجوال -->
...
            .footer { margin-top: 25px; font-size: 14px; color: #94a3b8; border-top: 1px solid #334155; padding-top: 15px; }
...
            <div class="footer">
                مطور الإضافة: <strong>خالد</strong> <br>
                انستقرام: <a href="https://instagram.com/YOUR_INSTA_USERNAME" target="_blank">@YOUR_INSTA_USERNAME</a>
            </div>
        </div>
        <script>
            function gen() {
...
                
                // إعادة الزر لشكله الطبيعي عند توليد رابط جديد
...
                out.select();
                out.setSelectionRange(0, 99999); // لتوافق الجوالات
...
// معالج البحث بالاسم وجلب الروابط
...
                const ep = epData.episodes[cleanId.split(':')[1]]?.find(e => e.episode_num == cleanId.split(':')[2]);
                if(ep) s = `${b}/series/${c.username}/${c.password}/${ep.id}.${ep.container_extension || 'mp4'}`;
            }
        }
        res.json({ streams: s ? [{ name: "khalid iptv", title: "📺 تشغيل من اشتراكك", url: s }] : [] });
